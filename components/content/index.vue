<template>
  <div id="listswiper" class="listswiper">
    <div
      :style="{top: getSwiperTop,bottom: getSwiperBottom,'background-color': (backgroundColor?backgroundColor:('#ffffff')),}"
    >
      <template>
        <slot name="custom-content">
          ...
          <swiper
            ...
            :style="{top: getSwiperTop,bottom: getSwiperBottom,'background-color': (backgroundColor?backgroundColor:(tabs[curIndex]&&tabs[curIndex].upCallbackFun?'#efeeee':'#ffffff')),}"
          >
            <!--首页-->
                ...
                >
                  ...
                  <div v-if="!connection" class="mescroll-empty">
                    ...
                    <p
                      ...
                      class="empty-tip"
                    >{{dealExceptionShow(emptyImg,'show')}}~</p>
...
import { isEmptyObject, formatDateTime, ui, } from "../_util";
...
export default {
  ...
  computed: {
    ...
    getSwiperTop() {
      ...
      return ui.dealPxToVw(head + ui.allHeadTopPx) + "vw";
    },
    getSwiperBottom() {
      // 红线的位置
    ...
    return {
      ...
      swiperOption: {
        ...
        on: {
          transitionEnd: () => {
            const activeIndex =
              this.swiper && this.swiper.activeIndex
      ...
      this.$emit(
        ...
        curPageData => {
          if (curPageData) {
            ...
            // 数据返回节点可能错误
            console.log(
              "%c 数据返回节点可能错误-返回data-curPageData",
              "color:red;",
              curPageData
            );
            console.log(
              "%c 注：--content组件upCallbackFun方法需要传入数组",
              "color:red;",
              curPageData
            );
  ...
  deactivated() {
    ...
    console.log(
      "%c this.tabs[0].mescroll",
      "color:green;",
      this.tabs[this.curIndex].mescroll
    );
  ...
  watch: {
    isMescrollDown(val) {
      this.tabs &&
        this.tabs[0] &&
        this.tabs[0].mescroll &&
        this.tabs[0].mescroll.lockDownScroll(val);
    },
    curIndex(val) {
      this.connection = true;
      this.$emit("input", val);
