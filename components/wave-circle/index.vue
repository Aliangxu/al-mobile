    ...
    <div v-if="!isLoadCircle" class="n22_wave_circle">
        <div class="circle_title">
            <div class="circleTitleeclass" :style='circleTitleestyle'>
                <!-- {{defaultNum}} -->
                <n22-amount :precision="precision" :value="defaultNum" :duration="800" transition></n22-amount>
            </div>
            <div class="circleTitleeclassd" :style='circleTitleestyled'>当前考试得分</div>
        </div>
        <div class="container">
...
export default {
  ...
  props: {
    ...
    boundaryUpColor: {//分界线以上的样式颜色
      type: String,
      default: '#FFFFFF'
    },
    boundaryDownColor: {//分界线以下的样式颜色
      type: String,
      default: '#FFFFFF'
    },
    boundary: {//分界线--例如当及格分用
      type: Number,
      default: 60
    },
    //2
  ...
  methods: {
    init(){
      if (isType('Number',this.value)&&this.value>=0&&this.value<=100) {
        ...
        if (String(this.value).indexOf(".")>-1&&this.precision==0) {
          ...
          console.log('%c this.num值异常，应为0-100的整数，当前值为','color:green;',this.value)
          return
      }
      //1
        ...
        var clearInt = setInterval(function() {
          try {
            if(marginTop>=0 && marginTop<=100){
              ...
              _this.$refs.wave.style.marginTop = "-"+((marginTop)*_this.offsetRate+_this.offset)+"vw"
