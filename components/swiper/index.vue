<template>
  <div
    class="n22-swiper"
    :class="{'n22-swiper-vertical': isVertical, 'n22-swiper-fade': !isSlide, 'disabled': !isInitial}"
  ...
  >
    <div class="n22-swiper-box">
      <div class="n22-swiper-container">
        <slot></slot>
      </div>
    </div>
    <div class="n22-swiper-indicators" :class="{'disabled': !hasDots}" v-if="oItemCount > 1 && hasDots">
      <template v-for="index in oItemCount">
        <div
          class="n22-swiper-indicator"
          :key="index"
          :class="{ 'n22-swiper-indicator-active': index - 1 === realIndex }"
          ></div>
      </template>
    </div>
  </div>
...
export default {
  ...
  */
  mounted() {
    this.ready = true
    this.$swiper = this.$el.querySelector('.n22-swiper-container')
    this.$swiperBox = this.$el.querySelector('.n22-swiper-box')
    this.$nextTick(() => {
      this.$_reInitItems()
      this.$_startPlay()
  ...
  methods: {
    ...
    $_backupItem(children) {
      ...
      if (children.length > 1 && this.isLoop) {
        const firstNodeCopy = this.$swiper.querySelector('.n22-swiper-item-first-copy')
        const lastNodeCopy = this.$swiper.querySelector('.n22-swiper-item-last-copy')
        firstNodeCopy && this.$swiper.removeChild(firstNodeCopy)
        lastNodeCopy && this.$swiper.removeChild(lastNodeCopy)

        firstNode.className += ' n22-swiper-item-first-copy'
        lastNode.className += ' n22-swiper-item-last-copy'
        if (this.isVertical) {
    ...
    $_translate(element, offset, animate = true) {
      if (!element) {
        warn('[n22-swiper] no element for translate')
        return
      }
...
.n22-swiper, .n22-swiper-box
  ...
  &.n22-swiper-fade
    .n22-swiper-item
      ...
      left 0
  &.n22-swiper-vertical
    .n22-swiper-container
      width 100%
      height auto
      box-orient vertical
      flex-direction column
    .n22-swiper-indicators
      flex-direction column
      right 20px
      left auto
      bottom auto
